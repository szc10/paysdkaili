 ...
 * Jajax接口集成file上传
 * Updataed by szc on 16/5/20
 * 1.重写了类模板引擎
 * 2.增加了类创建工厂
 * 3.修复了小的bug
 * 4.移除了原有的JajaxFile接口
 * * Updataed by szc on 16/8/24,
 * 1.重新编写了cookic的处理模块,允许了cookic跨域
 * 2.增加了 UImodel, UIacivity 处理类
 * 3.js的类继承模式
 * 4.删除其中的界面回弹的功能
 *  1.0.1
 *  优化事件管理器
...
Action = (function() {
    ...
    AddEvent(document, 'click', function(e) {
        for (var i in preventArr) {
            var target = getTarget(e);
            if (target.id != i) {
                if (target.getAttribute("onfocus") == "laymonth(this)" || target.parentNode.id == "time" || target.parentNode.id == "month_select") {
                    delete preventArr[i];
                } else {
                    var select = nextElementSibling(document.getElementById(i)) || document.getElementById("time_controller");
                    if (i.split("-")[0] == "accountentry") {
                        select = document.getElementById("account-entry-select");
                    }
                    preventArr[i].call(this, select); //回调
                    delete preventArr[i]; //清除缓存
                }
            }
        }
...
var Jmodel = function(newModelAction) {
    var modelAction = newModelAction;
    this.action = modelAction;
    var context = this.getContext();
    var state = null;
    this._init = function() {
        if (modelAction.init) {
            modelAction.init.apply(this, arguments);
        }
    }
    this._activite = function() {
        if (modelAction.activite) {
            modelAction.activite.apply(this, arguments);
        }
        context.style.display = "block";
    }
    this._sleep = function() {
        context.style.display = "none";
        if (modelAction.sleep) {
            modelAction.sleep.apply(this, arguments);
        } else
            return 0;
    }
    this._destroy = function() {
            //清楚UI缓冲区的内存
            var uimodel = document.getElementById(modelAction.modelName);
            document.getElementsByTagName('body')[0].removeChild(uimodel);
            delete test; //清理代码内存区
        }
        /**
         * 更新当前model的状态
...
Jmodel.prototype = {
    ...
    activite: function() {
        var state = "active";
        this._updateState(state);
        URLrouter.updateHash({ "currentmodel": this.action.modelName })
     ...
     */
    getContext: function() {
        var dom = document.getElementById(this.action.modelName);
        for (var i = 0; i < dom.childNodes.length; i++) {
...
function Transition(dom) {
    ...
    dom.style.transition = "all .3s ease-out";
